...
interface CreateRepostModalProps {
  ...
  export function CreateRepostModal({ onClose, remixPost }: CreateRepostModalProps) {
    const logged = aon.store((x) => x.session?.user);
    const imageProfURL = `http://localhost:8000/api${logged?.profimage}`;
    const imageRepostURLold = `http://localhost:8000/api${remixPost.postimage}`;
  ...
  const handleRemixModelChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    setVisibility(event.target.value);
    setRemixModel(event.target.value);
  };
  ...
  const handleImageRemix = async () => {
  
    if (remixPost.Remix||'true') {

const apiUrl = 'https://127.0.0.1:5001/controldraw';


 // Download the image from the provided URL
 const imageResponse = await fetch(imageRepostURL);
 const imageBlob = await imageResponse.blob();

// Function to get the selected model from the dropdown list

// Event handler for the button click
const handleButtonClick = () => {
  const promptInput = remixPrompt;
  const prompt = promptInput;

  const selectedModel = remixModel;
  ...
  const requestData = {
    ...
    console.log('remixdata',requestData);
  // Assuming you have a file input field in your HTML with id="imageInput"
  //const imageInput = document.getElementById('imageInput') as HTMLInputElement;
  if (selectedModel) {
    const imageFile = new FormData();
    imageFile.append('image', imageBlob);

    // Axios request
    axios.post(apiUrl, imageFile, {
      params: requestData,
      headers: {
...
// Assuming you have a button in your HTML with id="submitButton"
...
<div>
      <textarea
        placeholder="Enter Remix prompt text"
        value={remixPrompt}
      ...
      <select
        value={remixModel}
        onChange={handleRemixModelChange}
        className="ml-2 py-2 px-5 mt-2 bg-transparent border-2 border-white rounded-lg text-white"
      >
        {/* Options for the second dropdown */}
        <option className="bg-[var(--color-createpost)]" value="Dreamlike Photoreal">-- Photoreal --</option>
        <option className="bg-[var(--color-createpost)]" value="Ghibli Style">Ghibli Style</option>
        <option className="bg-[var(--color-createpost)]" value="Dreamlike Anime">Dreamlike Anime</option>
        <option className="bg-[var(--color-createpost)]" value="epiCRealism">epiCRealism</option>
        <option className="bg-[var(--color-createpost)]" value="Dreamlike Diffusion">Dreamlike Diffusion</option>
      ...
      >
        Remix
            ...
            <div className="flex">
                ...
                >
                {/* Options for the second dropdown */}
                    <option className="bg-[var(--color-createpost)]" value="">-- Visibility --</option>
                    <option className="bg-[var(--color-createpost)]" value="all">All</option>
